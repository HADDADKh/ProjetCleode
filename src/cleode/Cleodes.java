...
public class Cleodes implements AliveCallback, ZigbeeConnectorNodeEvents, ZigbeeConnectorEvents,
		LoadingProgressCallback {
	
	public static long getZplugEnergy() {
		return zplugEnergy;
	}


	public boolean isZplugOn() {
		return zplugOn;
	}

	private static long zplugEnergy;
	private boolean zplugOn;
	...
	protected ZigbeeConnectorEvents observer = new ZigbeeConnectorEvents() {
		@Override
		public void notifyReportAttribute(int nwkAddr, int epNumber, int clusterId, int seqNumber,
				final ReportAttribute[] attributes) {
			try {
				displayReportAttribute("Two", nwkAddr, epNumber, clusterId, attributes);
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
		...
		@Override
		public void notifyClusterSpecificCommand(int nwkAddr, int epNumber, int clusterId,
				int seqNumber, byte command, ClusterSpecCmdAttribute[] attributes) {
			displayClusterSpecificCommand("Two", nwkAddr, epNumber, clusterId, command, attributes);
		}
	};
	...
	protected ResponseCallBack<ZCLFrame> zclRespCallback = new ResponseCallBack<ZCLFrame>() {
		@Override
		public void responseReceived(int nwkAddr, int epNumber, int clusterId, int seqNumber,
				final ZCLFrame response) {
			System.out.printf(
					"=====> responseReceived ZCLFrame from 0x%04x Ep(%d) Cl(0x%04x) : %s \n",
					nwkAddr, epNumber, clusterId, (response == null) ? "Timeout expired !"
							: response.getHeader().getCmdIdentifier().name());
		}
	};
		...
		public void responseReceived(int nwkAddr, int epNumber, int clusterId, int seqNumber,
			...
			System.out
					.printf("=====> responseReceived ReadAttributeResponse from 0x%04x Ep(%d) Cl(0x%04x) : %s \n",
							nwkAddr, epNumber, clusterId, (responses == null) ? "Timeout expired !"
									: Integer.toString(responses.length));
			if (responses != null) {
				for (ReadAttributeResponse attr : responses) {
					...
					if (attr.getStatus() == Status.SUCCESS) {
						if (attr.getDataType() == DataType.CHARACTER_STRING) {
							...
							dataAsString = Boolean.toString(attr.getDataAsBoolean());
						} else if (attr.getDataType().isNumber()) {
							dataAsString = attr.getDataAsNumber().toString();
						}
						System.out.printf("AttrId(0x%04x) - Status(%s) - DataType(%s) : %s\n",
								attr.getIdentifier(), attr.getStatus(), attr.getDataType(),
								dataAsString);
		...
		public void responseReceived(int nwkAddr, int epNumber, int clusterId, int seqNumber,
			...
			if (responses != null) {
				for (ReadReportingConfigAttributeResponse attr : responses) {
					if (attr.getStatus() == Status.SUCCESS) {
						String reportChange = (attr.getDataType().isAnalog() ? String.format(
								"- Change(%d)", attr.getReportableChangeAsNumber().longValue())
								: "");
						System.out
								.printf("AttrId(0x%04x) - Direction(%s) - Status(%s) DataType(%s) - Min(%d) - Max(%d) %s\n",
										attr.getIdentifier(), attr.getDirection(),
										attr.getStatus(), attr.getDataType(),
										attr.getMinReportingInterval(),
										attr.getMaxReportingInterval(), reportChange);
					} else {
						System.out.printf("AttrId(0x%04x) - Direction(%s) - Status(%s)\n",
								attr.getIdentifier(), attr.getDirection(), attr.getStatus());
					}
				}
			}
			System.out.println();
	...
	protected ResponseCallBack<ClusterSpecCmdAttribute[]> zclClSpecCmdReader = new ResponseCallBack<ClusterSpecCmdAttribute[]>() {
		@Override
		public void responseReceived(int nwkAddr, int epNumber, int clusterId, int seqNumber,
				ClusterSpecCmdAttribute[] responses) {
			System.out
	...
	protected BindResponse zclBinder = new BindResponse() {
		...
		public void responseReceived(int nwkAddr, final BindCmd cmd, final Status status) {
			System.out.printf("=====> responseReceived %s Response from 0x%04x : Status(%s)  \n",
					cmd, nwkAddr, status);
	...
	void startConfigurationProgram() {
		boolean debug = false;
		...
		do {
			try {
				...
				if (line.equals("h")) {
					...
					System.out.println("'b' Read Attributes command from ZPLUG");
					System.out.println("'c' Configure Reporting command to ZRC and ZPLUG");
					System.out.println("'d' Discover Attributes command from ZPLUG");
					System.out.println("'e' Bind On/Off cluster : ZRC ==> ZPLUG");
					System.out.println("'f' Unbind On/Off cluster : ZRC ==> ZPLUG");
					...
					System.out.println("'m' Print the list of nodes with clusters");
					...
					System.out.println("'r' Read Attributes command from ZPLUG");
					System.out.println("'s' Specific Cluster command without response to ZPLUG");
					System.out.println("'t' Set Debug trace On/Off");
					System.out.println("'u' Specific Cluster command with response to ZPLUG");
					...
					System.out.println("'y' Force the ZPLUG to leave the network");
					System.out.println("'z' ZCL frame command");
					System.out.println("'A' Print the list of authorized devices");
					System.out
							.println("'B' Add the discovered nodes in the list of authorized devices");
					System.out.println("'C' Clear the list of authorized devices");
					System.out.println("'D' Remove the ZPlug from the list of authorized devices");
					System.out
					...
					System.out.println("'G' Print the network channel and a Pan ID");
					...
					if (file.exists() || file.isFile()) {
						System.out.printf("===============>Load UBee firmware : %s\n"
					...
					if (zplug != null) {
						this.zcs.sendReadAttributesCommand(
								this.zclReader,
								zplug.getNwkAddr(),
								zplug.getEpByInCluster(ClusterName.Basic_Attributes.getId()),
								...
								new Attribute[] {
										...
										Attribute.factory.newInstance(AttrConstants.DATE_CODE_ATTR),
										Attribute.factory
					...
					if (zplug != null) {
						...
						this.zcs.sendConfigureReportingCommand(this.zclReportWriter,
								zplug.getNwkAddr(),
						...
						this.zcs.sendConfigureReportingCommand(this.zclReportWriter,
								zplug.getNwkAddr(),
								zplug.getEpByInCluster(ClusterName.Simple_Metering.getId()),
								ClusterName.Simple_Metering.getId(),
								new ReportingConfigurationAttribute[] { attr1, attr2 });
					} else {
						System.out.println("No ZPlug in network");
					}
					...
					Node zplug = this.searchNode("ZPLUG");
					if (zplug != null) {
						this.zcs.sendDiscoverAttributesCommand(this.zclDiscover,
								zplug.getNwkAddr(),
								zplug.getEpByInCluster(ClusterName.Basic_Attributes.getId()),
								ClusterName.Basic_Attributes.getId(), 0x0000, 30, true);
					} else {
						System.out.println("No ZPLUG in network\n");
					}
					...
					if (zplug != null && zrc != null) {
						this.zcs.sendBindRequest(this.zclBinder, zrc.getNwkAddr(),
								zrc.getIEEEAddress(),
								zrc.getEpByOutCluster(ClusterName.On_Off.getId()),
								ClusterName.On_Off.getId(), zplug.getIEEEAddress(),
								zplug.getEpByInCluster(ClusterName.On_Off.getId()));
					...
					Node zrc = this.searchNode("ZRC");
					if (zplug != null && zrc != null) {
						this.zcs.sendUnbindRequest(this.zclBinder, zrc.getNwkAddr(),
								zrc.getIEEEAddress(),
								zrc.getEpByOutCluster(ClusterName.On_Off.getId()),
								ClusterName.On_Off.getId(), zplug.getIEEEAddress(),
								zplug.getEpByInCluster(ClusterName.On_Off.getId()));
					...
					if (list.size() > 0) {
						...
						this.zcs.sendBindingsListRequest(this.zclBinder, nwkAddrs);
					}
					...
					synchronized (this.mapOfNodes) {
						...
						for (Node node : this.mapOfNodes.values()) {
							...
							for (Link link : node.getLinks()) {
								sb.append(String.format(" 0x%04x", link.getChildNwkAddr()));
							}
							System.out.printf("Node %s %s  parent = 0x%04x  children =%s\n",
									this.formatNodeName(node), node.getIEEEAddress().toString(),
									node.getParentNwkAddr(), sb.toString());
						}
					}
					...
					for (Node node : this.zcs.getNodes()) {
						...
						for (EndPoint ep : node.getEndPoints()) {
							...
							for (Cluster inCl : ep.getInClusters()) {
								System.out.printf("    In cluster %s(0x%04x) \n", inCl.getName(),
										inCl.getId());
							}
							for (Cluster outCl : ep.getOutClusters()) {
								System.out.printf("    Out cluster %s(0x%04x) \n", outCl.getName(),
										outCl.getId());
							}
						}
					}
					...
					if (zrc != null) {
						...
						try {
							this.zces.subscribe(this.observer, zrc.getNwkAddr());
							System.out.printf("===========> subscribe on ReportAttributes to "
									+ zrc.toString());
						} catch (Exception e) {
					...
					if (zplug != null) {
						try {
							this.zces.subscribe(this.observer, zplug.getNwkAddr());
							System.out.printf("===========> subscribe on ReportAttributes to "
									+ zplug.toString());
						} catch (Exception e) {
							System.err.println("Exception:" + e.getMessage());
					...
					if (zplug != null) {
						ReadReportingConfigAttribute attr = ReadReportingConfigAttribute.factory
								.newInstance(ReportingDirection.ValuesReported);
						attr.setIdentifier(AttrConstants.ON_OFF_ATTR);
						this.zcs.sendReadReportingConfigurationCommand(this.zclReportReader,
								zplug.getNwkAddr(),
								zplug.getEpByInCluster(ClusterName.On_Off.getId()),
								ClusterName.On_Off.getId(),
						...
						this.zcs.sendReadReportingConfigurationCommand(this.zclReportReader,
								...
								zplug.getEpByInCluster(ClusterName.Simple_Metering.getId()),
								ClusterName.Simple_Metering.getId(),
								new ReadReportingConfigAttribute[] { attr2, attr1 });
					} else {
						System.out.println("No ZPlug in network");
					}
					Node zrc = this.searchNode("ZRC");
					...
					Node zplug = this.searchNode("ZPLUG");
					if (zplug != null) {
						this.zcs.sendReadAttributesCommand(this.zclReader, zplug.getNwkAddr(),
								zplug.getEpByInCluster(ClusterName.On_Off.getId()),
								ClusterName.On_Off.getId(), new Attribute[] { Attribute.factory
					...
					debug = !debug;
					ZigbeeConnector.singleton.getZigbeeSettingsServices().setDebugTrace(debug);
					System.out.printf("Debug trace : %s\n", Boolean.toString(debug));
				} else if (line.equals("u")) {
					Node zplug = this.searchNode("ZPLUG");
					if (zplug != null) {
						this.zcs.sendClusterSpecificCommand(this.zclClSpecCmdReader,
								zplug.getNwkAddr(),
								zplug.getEpByInCluster(ClusterName.Groups.getId()),
								ClusterName.Groups.getId(),
								AttrConstants.GET_GROUP_MEMBERSHIP_COMMAND, new byte[] { 0x00 });
						...
						this.zcs.sendClusterSpecificCommandExt(this.zclClSpecCmdReader,
								zplug.getNwkAddr(),
								zplug.getEpByInCluster(ClusterName.Groups.getId()),
								ClusterName.Groups.getId(),
					...
					System.out.println("ZCL library version : " + zcs.getVersion());
					...
					if (zplug != null) {
						...
						attr.setDataAsString(String.format("ZPLUG_%s", df.format(new Date())));
						this.zcs.sendWriteAttributesCommand(this.zclWriter, zplug.getNwkAddr(),
								zplug.getEpByInCluster(ClusterName.Basic_Attributes.getId()),
								ClusterName.Basic_Attributes.getId(), new WriteAttribute[] { attr });
					} else {
						System.out.println("No ZPLUG in network");
					}
					...
					if (zplug != null) {
						...
						frame.getHeader().getFrameControl().setFrameType(FrameType.EntireProfile);
						frame.getHeader().getFrameControl().setDirection(Direction.ClientToServer);
						frame.getHeader().getFrameControl()
								.setManufacturerCode(ManufacturerCode.NotIncludeInZCLFrame);
						frame.getHeader().getFrameControl()
								.setResponseStatus(ResponseStatus.ResponseReturned);
						ReadAttributesPayload payload = frame.createReadAttributesPayload();
						Attribute attr = Attribute.factory
								.newInstance(AttrConstants.MODEL_IDENTIFIER_ATTR);
						payload.addReadAttribute(attr);
						this.zcs.sendZCLFrameCommand(this.zclRespCallback, zplug.getNwkAddr(),
								zplug.getEpByInCluster(ClusterName.Basic_Attributes.getId()),
								ClusterName.Basic_Attributes.getId(), frame);
					...
					for (IEEEAddress device : this.zas.getAllAuthorizedDevices()) {
						System.out.println(" " + device.toSmallString());
					}
				} else if (line.equals("B")) {
					for (Node node : mapOfNodes.values()) {
						System.out.printf("Add authorized device %s : %s \n",
								node.getIEEEAddress(), Boolean.toString(this.zas
										.addAuthorizedDevice(node.getIEEEAddress())));
					}
				} else if (line.equals("C")) {
					...
					Node zplug = this.searchNode("ZPLUG");
					if (zplug != null) {
						System.out.printf("Remove authorized device %s : %s \n", zplug
								.getIEEEAddress(), Boolean.toString(this.zas
								.removeAuthorizedDevice(zplug.getIEEEAddress())));
					}
				} else if (line.equals("E")) {
					...
					this.zas.initAuthorizedDevices(list.toArray(new IEEEAddress[list.size()]));
					// Start UBEE
					System.out.printf("Start UBEE %s \n",
							Boolean.toString(this.zcs.startUbee(this)));
				} else if (line.equals("F")) {
					...
					System.out.printf("Network channel = %d - Pan ID = 0x%04x \n",
							this.zcs.getNetworkChannel(), this.zcs.getNetworkPanId());
				} else if (line.equals("O")) {
					int duration = 300;
...
//		}
		...
		this.zcs.setLicenseKey("QAUS5Q9801QC3DLX5X646J63L");
	...
	public void stopZCL() {
		...
		System.out.println("StopUbee = " + Boolean.toString(this.zcs.stopUbee()));
	...
	public void notifyReportAttribute(int nwkAddr, int epNumber, int clusterId, int seqNumber,
			final ReportAttribute[] attributes) {
		try {
			this.displayReportAttribute("One", nwkAddr, epNumber, clusterId, attributes);
		} catch (IOException e) {
	...
	public void notifyClusterSpecificCommand(int nwkAddr, int epNumber, int clusterId,
			int seqNumber, byte command, ClusterSpecCmdAttribute[] attributes) {
		this.displayClusterSpecificCommand("One", nwkAddr, epNumber, clusterId, command, attributes);
	}

	private void displayReportAttribute(final String name, int nwkAddr, int epNumber,
			int clusterId, final ReportAttribute[] attributes) throws IOException {
		String[] tmp;
			...
			if (clusterId == ClusterName.On_Off.getId()) {
				System.out
						.printf("=====> Obs(%s) - notifyReportAttribute from 0x%04x Location(%s) Cl(0x%04x) : On/Off State = %s \n",
								name, nwkAddr, addressOfNodes.get(nwkAddr), clusterId,
				...
				System.out
						.printf("=====> Obs(%s) - notifyReportAttribute from 0x%04x Location(%s) Cl(0x%04x) : Temperature = %.2f dC \n",
								name, nwkAddr, addressOfNodes.get(nwkAddr), clusterId, degree);
				...
				tmp = new String[] { addressOfNodes.get(nwkAddr), "" + attr.getDataAsInteger(),
...
//				ZMovewriter.flush();
				...
				tmp = new String[] { addressOfNodes.get(nwkAddr), "" + attr.getDataAsLong(),
						timeCell.format(new Date()) };
				...
				if (attr.getIdentifier() == AttrConstants.CURRENT_SUMMATION_DELIVERED_ATTR) {
					...
					System.out
							.printf("=====> Obs(%s) - notifyReportAttribute from 0x%04x Location(%s) Cl(0x%04x) : Consommation cumulee = %.2f kWh \n",
									name, nwkAddr, addressOfNodes.get(nwkAddr), clusterId, kwh);
					...
					zplugEnergy= attr.getDataAsLong();
					...
					// TODO important data
//					ZPlugwriter.writeNext(tmp, false);
//					ZPlugwriter.flush();
				} else {
					System.out
				...
				System.out.println("***************" + ClusterName.valueOf(clusterId) + "    "
						+ addressOfNodes.get(nwkAddr));
			}
		}
	}

	private void displayClusterSpecificCommand(final String name, int nwkAddr, int epNumber,
			int clusterId, byte command, ClusterSpecCmdAttribute[] attributes) {
		try {
			StringBuilder sb = new StringBuilder(":");
			for (ClusterSpecCmdAttribute attr : attributes) {
				...
				sb.append(String.format("%s (%s)", str, attr.getDataType().name()));
			}
			System.out
					.printf("=====> Obs(%s) - notifyClusterSpecificCommand from 0x%04x Location(%s) Cl(%s) - Command = 0x%02x %s \n",
							name, nwkAddr, addressOfNodes.get(nwkAddr),
							ClusterName.valueOf(clusterId), command, sb.toString());
			int i = (int) Integer.parseInt(attributes[0].getDataAsNumber().toString());

			// i inform the state of the door or the presence of leak, if 1 the door is open, if 0
			// the door is close
			if (i == 33)
				i = 1; // Door is open
			else if (i == 32)
			...
			if (addressOfNodes.get(nwkAddr) != null) {
				String[] tmp = { addressOfNodes.get(nwkAddr), "" + i, timeCell.format(new Date()) };
				if (mapOfNodes.get(nwkAddr).getModelIdentifier().equals("ZDOOR")) {
					// TODO important data
//					ZDOORwriter.writeNext(tmp, false);
//					ZDOORwriter.flush();
				} else if (mapOfNodes.get(nwkAddr).getModelIdentifier().equals("ZLEAK")) {
					// TODO important data
...
//					ZLeakwriter.flush();
	...
	private void displayBinding(final Node node, final Binding binding) {
		if (binding.isDstGroupAddr()) {
			System.out.printf("Binding of %s 0x%04x : %s Ep(%d) Cl(%s) Group 0x%04x\n", node
					.getModelIdentifier(), node.getNwkAddr(), binding.getSrcIEEEAddr().toString(),
					binding.getSrcEpNumber(), ClusterName.valueOf(binding.getClusterId()), binding
							.getDstGroupAddr());
		} else {
			Node destNode = searchNode(binding.getDstIEEEAddr());
			System.out.printf("Binding of %s 0x%04x : %s Ep(%d) Cl(%s) ==> %s %s Ep(%d)\n", node
					.getModelIdentifier(), node.getNwkAddr(), binding.getSrcIEEEAddr().toString(),
					binding.getSrcEpNumber(), ClusterName.valueOf(binding.getClusterId()),
					(destNode != null ? destNode.getModelIdentifier() : "Unknown"), binding
							.getDstIEEEAddr().toString(), binding.getDstEpNumber());
		}
	}
	...
	public void ubeeStarted() {
		...
		System.err.printf("UBee License validity for %s : %s", this.zcs.getCoordinator(),
				Boolean.toString(this.zcs.isLicenseValid()));
//		date = new Date();
//		String fileName = ft.format(date).toString() + ".csv";
//		try {
//			ZDOORwriter = new CSVWriter(new FileWriter("ZDoor_" + fileName), ',');
//			ZLeakwriter = new CSVWriter(new FileWriter("ZLeak_" + fileName), ',');
//			ZMovewriter = new CSVWriter(new FileWriter("ZMove_" + fileName), ',');
//			ZLumwriter = new CSVWriter(new FileWriter("ZLum_" + fileName), ',');
//			ZPlugwriter = new CSVWriter(new FileWriter("ZPlug_" + fileName), ',');
//
//		} catch (IOException e) {
//			// TODO Auto-generated catch block
//			e.printStackTrace();
//		}
	...
	private boolean isBindingAlreadyCreated(final Node srcNode, final ClusterName cluster,
			final Node dstNode) {
		for (Binding binding : srcNode.getBindings()) {
			if (binding.getClusterId() == cluster.getId() && binding.isDstGroupAddr() != true
					&& srcNode.getIEEEAddress().equals(binding.getSrcIEEEAddr())
					&& dstNode.getIEEEAddress().equals(binding.getDstIEEEAddr())
					&& srcNode.getEpByInCluster(cluster.getId()) == binding.getSrcEpNumber()
					&& dstNode.getEpByOutCluster(cluster.getId()) == binding.getDstEpNumber()) {
				return true;
			}
		}
		return false;
	...
	private void createBindingsForZPLug(final Node node) {
		...
		if (ubee != null) {
			...
			if (isBindingAlreadyCreated(node, ClusterName.On_Off, ubee) != true) {
				this.zcs.sendBindRequest(this.zclBinder, node.getNwkAddr(), node.getIEEEAddress(),
				...
				System.out.printf("===========> Create Binding On/Off between %s --> UBEE \n",
						node.toString());
			}
			...
			if (isBindingAlreadyCreated(node, ClusterName.Simple_Metering, ubee) != true) {
				this.zcs.sendBindRequest(this.zclBinder, node.getNwkAddr(), node.getIEEEAddress(),
						node.getEpByInCluster(ClusterName.Simple_Metering.getId()),
						ClusterName.Simple_Metering.getId(), ubee.getIEEEAddress(),
						ubee.getEpByOutCluster(ClusterName.Simple_Metering.getId()));
				System.out.printf(
						"===========> Create Binding Simple Metering between %s --> UBEE \n",
						node.toString());
			}
		}
	}
	...
	private void createBindingsForZRC(final Node node) {
		...
		if (ubee != null) {
			// Check if the binding is already created
			if (isBindingAlreadyCreated(node, ClusterName.Temperature_Measurement, ubee) != true) {
				this.zcs.sendBindRequest(this.zclBinder, node.getNwkAddr(), node.getIEEEAddress(),
						node.getEpByInCluster(ClusterName.Temperature_Measurement.getId()),
						ClusterName.Temperature_Measurement.getId(), ubee.getIEEEAddress(),
			...
			if (isBindingAlreadyCreated(node, ClusterName.Alarms, ubee) != true) {
				...
				System.out.printf("===========> Create Binding Alarms between ZRC %s --> UBEE \n",
						node.toString());
			}
		}
	}
	...
	private void createBindingsForZDoor(final Node node) {
		...
		if (ubee != null) {
			...
			if (isBindingAlreadyCreated(node, ClusterName.IAS_Zone, ubee) != true) {
				...
				System.out.printf(
						"===========> Create Binding ZDoor Measurement between %s --> UBEE \n",
						node.toString());
			}
			...
			if (isBindingAlreadyCreated(node, ClusterName.Alarms, ubee) != true) {
				this.zcs.sendBindRequest(this.zclBinder, node.getNwkAddr(), node.getIEEEAddress(),
						...
						ubee.getEpByOutCluster(ClusterName.Alarms.getId()));
				System.out.printf(
						"===========> Create Binding Alarms between ZDoor %s --> UBEE \n",
						node.toString());
			}
		}
	}
	...
	private void createBindingsForZMove(final Node node) {
		...
		if (ubee != null) {
			// Check if the binding is already created
			if (isBindingAlreadyCreated(node, ClusterName.Occupancy_Sensing, ubee) != true) {
				this.zcs.sendBindRequest(this.zclBinder, node.getNwkAddr(), node.getIEEEAddress(),
						node.getEpByInCluster(ClusterName.Occupancy_Sensing.getId()),
						ClusterName.Occupancy_Sensing.getId(), ubee.getIEEEAddress(),
						ubee.getEpByOutCluster(ClusterName.Occupancy_Sensing.getId()));
				System.out.printf(
						"===========> Create Binding ZMove Measurement between %s --> UBEE \n",
						node.toString());
			}
			...
			if (isBindingAlreadyCreated(node, ClusterName.Alarms, ubee) != true) {
				this.zcs.sendBindRequest(this.zclBinder, node.getNwkAddr(), node.getIEEEAddress(),
						...
						ubee.getEpByOutCluster(ClusterName.Alarms.getId()));
				System.out.printf(
						"===========> Create Binding Alarms between Zmove %s --> UBEE \n",
						node.toString());
			}
		}
	}
		...
		if (ubee != null) {
			...
			if (isBindingAlreadyCreated(node, ClusterName.IAS_Zone, ubee) != true) {
				this.zcs.sendBindRequest(this.zclBinder, node.getNwkAddr(), node.getIEEEAddress(),
						...
						ubee.getEpByOutCluster(ClusterName.IAS_Zone.getId()));
				System.out.printf(
						"===========> Create Binding Zleak Measurement between %s --> UBEE \n",
						node.toString());
			}
			...
			if (isBindingAlreadyCreated(node, ClusterName.Alarms, ubee) != true) {
				this.zcs.sendBindRequest(this.zclBinder, node.getNwkAddr(), node.getIEEEAddress(),
						...
						ubee.getEpByOutCluster(ClusterName.Alarms.getId()));
				System.out.printf(
						"===========> Create Binding Alarms between Zleak %s --> UBEE \n",
						node.toString());
			}
		}
	}
	...
	private void createBindingsForZLum(final Node node) {
		...
		if (ubee != null) {
			// Check if the binding is already created
			if (isBindingAlreadyCreated(node, ClusterName.Illuminance_Measurement, ubee) != true) {
				this.zcs.sendBindRequest(this.zclBinder, node.getNwkAddr(), node.getIEEEAddress(),
						node.getEpByInCluster(ClusterName.Illuminance_Measurement.getId()),
						ClusterName.Illuminance_Measurement.getId(), ubee.getIEEEAddress(),
						ubee.getEpByOutCluster(ClusterName.Illuminance_Measurement.getId()));
				System.out.printf(
						"===========> Create Binding Zlum Measurement between %s --> UBEE \n",
						node.toString());
			}
			...
			if (isBindingAlreadyCreated(node, ClusterName.Alarms, ubee) != true) {
				this.zcs.sendBindRequest(this.zclBinder, node.getNwkAddr(), node.getIEEEAddress(),
						...
						ClusterName.Alarms.getId(), ubee.getIEEEAddress(),
						ubee.getEpByOutCluster(ClusterName.Alarms.getId()));
				System.out.printf("===========> Create Binding Alarms between Zlum %s --> UBEE \n",
						node.toString());
			}
		}
	}
	...
	protected void sendReadLocationDescription(final Node node) {
		Attribute attr = Attribute.factory.newInstance(AttrConstants.LOCATION_DESCRIPTION_ATTR);
		int epNumber = node.getEpByInCluster(ClusterName.Basic_Attributes.getId());
		this.zcs.sendReadAttributesCommand(this.zclReader, node.getNwkAddr(), epNumber,
				ClusterName.Basic_Attributes.getId(), new Attribute[] { attr });
	}
	...
	/**
	 * 
	 * @param nwkAddrInHexFormat
	 *            : adresse mac of the sensor tha you can get in console
	 * @param cmd
	 *            : the command to send. cmd equal "on" or "off"
	 * @author Fahem Belili Fahem (reuse and code modification) <br/>
	 * <br/>
	 *         20 march 2015
	 */
	public void sendCommandZPlugSwitchOnOff(String nwkAddrInHexFormat, String cmd) {
		nwkAddrInHexFormat = nwkAddrInHexFormat.replace("\n", "").trim();
		if (!nwkAddrInHexFormat.startsWith("0x")) {
			System.err.println("ERROR : '" + nwkAddrInHexFormat
					+ "' is not well formed as nwk Addr in Hexadecimal Format. "
		...
		System.out.println("INFO : '" + nwkAddrInHexFormat + "'  "
				+ Thread.currentThread().getStackTrace()[1].getClassName() + "/"
				+ Thread.currentThread().getStackTrace()[1].getMethodName() + '@'
				+ Thread.currentThread().getStackTrace()[1].getLineNumber());

		Node zplug = this.searchNode((int) Long.parseLong(nwkAddrInHexFormat.substring(2), 16));
		if (zplug != null) {
			...
			ReadAttributesPayload payload = frame.createReadAttributesPayload();
			Attribute attr = Attribute.factory.newInstance(AttrConstants.MODEL_IDENTIFIER_ATTR);
			payload.addReadAttribute(attr);
			this.zcs.sendZCLFrameCommand(this.zclRespCallback, zplug.getNwkAddr(),
					zplug.getEpByInCluster(ClusterName.Basic_Attributes.getId()),
					ClusterName.Basic_Attributes.getId(), frame);
			...
			if (cmd.trim().equalsIgnoreCase("off"))
				frame.getHeader().setCmdIdentifierAsValue(AttrConstants.OFF_COMMAND);
			else if (cmd.trim().equalsIgnoreCase("on"))
				frame.getHeader().setCmdIdentifierAsValue(AttrConstants.ON_COMMAND);

			this.zcs.sendZCLFrameCommand(this.zclRespCallback, zplug.getNwkAddr(),
					zplug.getEpByInCluster(ClusterName.On_Off.getId()), ClusterName.On_Off.getId(),
					frame);
		}
	}

	public static Cleodes getInstance() {
		return SingletonHolder.INSTANCE;
	}

	private static class SingletonHolder {
		private final static Cleodes INSTANCE = new Cleodes();
	}

	// public static void main(String[] args) {
//	// TestZCL zcl = new TestZCL();
...
//	// System.out.println();
//	// String nwkAddr = "0x0EAE";
//	// zcl.sendCommandZPlugSwitchOnOff(nwkAddr, "on");
//		 Cleodes cleode= Cleodes.getInstance();
////		 
//	 System.out.println(cleode.getZplugEnergy());
////
//	
//}
}
